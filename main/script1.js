...
function region() {
    if (locations.park === true) {
        ...
        // Update beg button state
        if (data.timerColdownActive) {
        ...
        shopContainer.style.display = locations.shop ? "block" : "none";
...
function begBtn () {
    ...
    if (locations.park === true && !data.timerColdownActive) {
        ...
        //timer
        const clickCooldown = 10;
        let timeleft = clickCooldown;

        data.timerColdownActive = true;
        activeBeg.disabled = true;
        ...
        const timer = setInterval(() => {
            ...
            if (!locations.park) {
                ...
                status.textContent = "Can't beg here";
                return;
            }
            ...
            if (timeleft > 0) {
                ...
                clearInterval(timer);
                activeBeg.disabled = false;
                data.timerColdownActive = false;
                status.textContent = "ready to beg";
            }
        }, 1000);
    } else if (locations.park !== true){
        status.textContent = "Can't beg here";
    } else if (data.timerColdownActive) {
        status.textContent = "Can't beg here";
    }
